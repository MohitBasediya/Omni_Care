...
import { setformData } from "../../store/userSlice.js";
...
function Salon_Registration() {
	...
	useEffect(()=>{
		if(!location.search){
			...
			if(status==='true'){
				...
				console.log("providerdata : ",providerData.toObject());
	...
	useEffect(()=>{
		const  serviceProviderServices = async(e)=>{
			var result = await axios.get(requestedURLForServiceProvider + `/getproviderservice?Service_type=${'Salon'}`);
					...
					<div className="col-md-7 col-12" >
						<div class="image-container">
						   ...
						   <div className="button_div">
							...
							<button type="submit" style={{ background: "black", color: 'white' }} className="fxt-btn" onClick={() => handleClick("salon")}>Salon</button><br />
					...
					<div className="col-md-5 col-12 px-5 py-3" style={{ background: "#F9F5F4" }}>
						<div className="maindiv" style={{background:'#FFB649'}}>
							<form method="post" onSubmit={submitData} encType="multipart/form-data">
								<div className="from1">
									<h1 className="h1" >Salon Registration</h1>
										<div>
														...
														return spservices.Primary.map((primaryService) => (															
															<li>
																<div className='w-100 d-flex justify-content-between py-2'>
																	<h5>{primaryService.ServiceName}</h5>
																    <h5>{primaryService.ServicePrice}</h5>
																</div>   
															</li>
														));
														})
												...
												<ul>
													{
														...
														return spservices.Secondary.map((secondaryService) => (															
															<li>
																<div className='w-100 d-flex justify-content-between py-2'>
																	<h5>{secondaryService.ServiceName}</h5>
																    <h5>{secondaryService.ServicePrice}</h5>
																</div>   
															</li>
														));
														})
												...
												<ul>
													{
														...
														return spservices.Tertiary.map((tertiaryService) => (															
															<li>
																<div className='w-100 d-flex justify-content-between py-2'>
																	<h5>{tertiaryService.ServiceName}</h5>
																    <h5>{tertiaryService.ServicePrice}</h5>
																</div>   
															</li>
														));
														})

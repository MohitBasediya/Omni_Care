...
import {Agency_Price_Modal} from "./Agency_Price_Modal.js";
...
function ServiceProvider_Modal(props){
    const location = useLocation();
    const [checked,setChecked]=useState(false);
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const [agencyPrice,setAgencyPrice] = useState(false);
    ...
    const handlePayment = async()=>{
        try{
            ...
            console.log('detail : ',providerData);
             axios.post(`${requestedURLForServiceProvider}/payment`,formData).then((response)=>{
...
export default ServiceProvider_Modal;